...
if ($db_connection) {
    if (strcmp($sort_by, "score") == 0 || strcmp($sort_by, "average_salary") == 0) {
        if ($state == -1) {
            $query = $db_connection->prepare("SELECT s.name, .2 * (1 - 1 / ((s.salary_twentyfive + s.salary_seventyfive) / 2 / ((s.in_state_tuition + s.out_of_state_tuition) / 2)) / 10) + .1 * (1 - 1 / (100000 / ((s.in_state_tuition + s.out_of_state_tuition + 2 * s.average_student_debt) / 4))) + .1 * st.score / 100 + .3 * s.graduation_rate + .3 * s.retention_rate AS 'score', s.city, s.in_state_tuition, s.out_of_state_tuition, (s.salary_twentyfive + s.salary_seventyfive) / 2 AS 'average_salary', st.name AS 'state' FROM schools s, located l, states st WHERE s.in_state_tuition > ? AND s.in_state_tuition < ? AND s.out_of_state_tuition > ? AND s.out_of_state_tuition < ? AND (s.salary_twentyfive + s.salary_seventyfive) / 2 > ? AND (s.salary_twentyfive + s.salary_seventyfive) / 2 < ? AND s.name = l.school AND l.state_code = st.code ORDER BY " . $sort_by . " DESC LIMIT ?;");
